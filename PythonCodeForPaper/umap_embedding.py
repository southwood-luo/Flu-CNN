...
def plot_umap(adata, n_neighbors, arg, namespace='6interlayer_fc1in'):
    ...
    # sc.pl.umap(adata, color='NA', size=20,
    #            save=f'figures/interlayer/{arg.seg}_{namespace}_{arg.data_type}_na{n}.png')
    # sc.pl.umap(adata, color='location',
    #            save=f'figures/{arg.seg}_{namespace}_{arg.data_type}_location{n}.png')
    # sc.pl.umap(adata, color='louvain',
    #            save=f'figures/{arg.seg}_{namespace}_{arg.data_type}_louvain{n}.png')
...
if __name__ == '__main__':
    ...
    # args.path = f'data/3parts/3parts_test_fa/3parts_8seg_unmafft_protein_{seg}_test.fa'
    ...
    #     host_group = host_group.capitalize()
    #     subtype = strain_info.split('|')[3]
    #     host_group_list.append(host_group)
    #     ha_list.append(subtype[:subtype.find('N')])
    #     na_list.append(subtype[subtype.find('N'):])
    #
    # obs = {'Host':host_group_list,'HA':ha_list,'NA':na_list}
    # save_dict(obs,'results/internal_layer/obs')
    tt = list()
    ...
    # diff_frame_mean = pd.read_csv(f'diff_frame_mean_{args.data_type}_{seg}.csv')
    # obs = np.load(f'obs_protein_{seg}.npy', allow_pickle=True).item()
    # HA = [ha[:ha.find('N')] for ha in obs['subtype']]
    # obs['subtype'] = HA
    #
    # adata = AnnData(diff_frame_mean, dtype='float64')
    # for key in obs:
    #     adata.obs[key] = obs[key]
    # adata = adata[
    #     np.logical_or.reduce((
    #         adata.obs['host'] == 'human',
    #         adata.obs['host'] == 'avian',
    #         adata.obs['host'] == 'swine',
    #     ))
    # ]
    n_neighbors = 10
